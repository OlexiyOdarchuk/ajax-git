...
enum {
  ...
  DECLARE_VMA_BIT(RAND_READ,
                  16),           /* App will not benefit from clustered reads */
  ...
  DECLARE_VMA_BIT(MIXEDMAP,
                  28), /* Can contain struct page and pure PFN pages */
...
#define VMA_STACK_INCOMPLETE_SETUP \
  append_vma_flags(VMA_STACK_EARLY, VMA_RAND_READ_BIT, VMA_SEQ_READ_BIT)

#define TASK_EXEC_BIT \
  ((current->personality & READ_IMPLIES_EXEC) ? VMA_EXEC_BIT : VMA_READ_BIT)

/* Common data flag combinations */
#define VMA_DATA_FLAGS_TSK_EXEC                                             \
  mk_vma_flags(VMA_READ_BIT, VMA_WRITE_BIT, TASK_EXEC_BIT, VMA_MAYREAD_BIT, \
               VMA_MAYWRITE_BIT, VMA_MAYEXEC_BIT)
#define VMA_DATA_FLAGS_NON_EXEC                                                \
  mk_vma_flags(VMA_READ_BIT, VMA_WRITE_BIT, VMA_MAYREAD_BIT, VMA_MAYWRITE_BIT, \
               VMA_MAYEXEC_BIT)
#define VMA_DATA_FLAGS_EXEC                                                \
  mk_vma_flags(VMA_READ_BIT, VMA_WRITE_BIT, VMA_EXEC_BIT, VMA_MAYREAD_BIT, \
               VMA_MAYWRITE_BIT, VMA_MAYEXEC_BIT)
...
#define VMA_STACK_FLAGS \
  append_vma_flags(VMA_STACK_DEFAULT_FLAGS, VMA_STACK_BIT, VMA_ACCOUNT_BIT)
...
#define VMA_SPECIAL_FLAGS \
  mk_vma_flags(VMA_IO_BIT, VMA_DONTEXPAND_BIT, VMA_PFNMAP_BIT, VMA_MIXEDMAP_BIT)
 ...
 */
#define VMA_REMAP_FLAGS \
  mk_vma_flags(VMA_IO_BIT, VMA_PFNMAP_BIT, VMA_DONTEXPAND_BIT, VMA_DONTDUMP_BIT)
...
struct vm_operations_struct {
   ...
   * Context: User context.  May sleep.  Caller holds mmap_lock.
   */
  int (*mapped)(unsigned long start,
                unsigned long end,
                pgoff_t pgoff,
 ...
 */
#define mk_vma_flags(...)                                  \
  __mk_vma_flags(EMPTY_VMA_FLAGS, COUNT_ARGS(__VA_ARGS__), \
 ...
 *              VMA_ACCOUNT_BIT);
 */
#define append_vma_flags(flags, ...)             \
  __mk_vma_flags(flags, COUNT_ARGS(__VA_ARGS__), \
                 (const vma_flag_t[]){__VA_ARGS__})
...
static __always_inline vma_flags_t
__mk_vma_flags_from_masks(size_t count, const vma_flags_t* masks) {
 ...
 */
static __always_inline vma_flags_t
vma_flags_diff_pair(const vma_flags_t* flags, const vma_flags_t* flags_other) {
  vma_flags_t dst;
...
static __always_inline bool vma_flags_same_pair(
 ...
 *		VMA_DONTEXPAND_BIT, VMA_DONTDUMP_BIT)) { ... }
 */
#define vma_test_any(vma, ...) vma_test_any_mask(vma, mk_vma_flags(__VA_ARGS__))
 ...
 * otherwise.
 */
static __always_inline bool vma_test_single_mask(
...
static __always_inline bool vma_desc_test_any_mask(
...
static __always_inline bool vma_desc_test_all_mask(
    const struct vm_area_desc* desc,
    vma_flags_t flags) {
...
static inline struct vm_area_struct* vma_iter_next_range(
...
#else
static inline bool vma_is_shmem(const struct vm_area_struct* vma) {
  return false;
}
static inline bool vma_is_anon_shmem(const struct vm_area_struct* vma) {
  return false;
}
#endif
...
vm_fault_t do_set_pmd(struct vm_fault* vmf,
                      struct folio* folio,
                      struct page* page);
void set_pte_range(struct vm_fault* vmf,
                   struct folio* folio,
                   struct page* page,
                   unsigned int nr,
                   unsigned long addr);
...
void unpin_user_pages_dirty_lock(struct page** pages,
                                 ...
                                 bool make_dirty);
void unpin_user_page_range_dirty_lock(struct page* page,
                                      unsigned long npages,
                                      bool make_dirty);
...
static inline int folio_xchg_access_time(struct folio* folio, int time) {
  ...
  last_time = folio_xchg_last_cpupid(folio, time >> PAGE_ACCESS_TIME_BUCKETS);
...
struct folio* vm_normal_folio(struct vm_area_struct* vma,
                              unsigned long addr,
                              pte_t pte);
...
void zap_special_vma_range(struct vm_area_struct* vma,
                           unsigned long address,
                           unsigned long size);
void zap_vma_range(struct vm_area_struct* vma,
                   unsigned long address,
                   unsigned long size);
/**
...
int generic_access_phys(struct vm_area_struct* vma,
                        unsigned long addr,
                        void* buf,
                        int len,
                        int write);
...
static inline int fixup_user_fault(struct mm_struct* mm,
                                   unsigned long address,
                                   unsigned int fault_flags,
                                   bool* unlocked) {
  /* should never happen if there's no MMU */
...
extern int access_process_vm(struct task_struct* tsk,
                             unsigned long addr,
                             void* buf,
                             int len,
...
extern int copy_remote_vm_str(struct task_struct* tsk,
                              unsigned long addr,
                              void* buf,
                              int len,
                              unsigned int gup_flags);
...
long pin_user_pages(unsigned long start,
                    ...
                    unsigned int gup_flags,
                    struct page** pages);
long get_user_pages_unlocked(unsigned long start,
                             unsigned long nr_pages,
                             struct page** pages,
                             unsigned int gup_flags);
long pin_user_pages_unlocked(unsigned long start,
                             unsigned long nr_pages,
                             struct page** pages,
                             unsigned int gup_flags);
...
int get_user_pages_fast(unsigned long start,
                        ...
                        unsigned int gup_flags,
                        struct page** pages);
int pin_user_pages_fast(unsigned long start,
                        int nr_pages,
                        unsigned int gup_flags,
                        struct page** pages);
...
int __account_locked_vm(struct mm_struct* mm,
                        unsigned long pages,
                        bool inc,
...
#define MM_CP_UFFD_RWP_ALL (MM_CP_UFFD_RWP | MM_CP_UFFD_RWP_RESOLVE)
...
extern int mprotect_fixup(struct vma_iterator* vmi,
 ...
 */
int get_user_pages_fast_only(unsigned long start,
                             int nr_pages,
                             unsigned int gup_flags,
                             struct page** pages);
...
static inline unsigned long get_mm_counter_sum(struct mm_struct* mm,
                                               int member) {
...
static inline void add_mm_counter(struct mm_struct* mm,
                                  int member,
                                  long value) {
...
extern pte_t* get_locked_pte(struct mm_struct* mm,
                             unsigned long addr,
                             spinlock_t** ptl);

#ifdef __PAGETABLE_P4D_FOLDED
static inline int __p4d_alloc(struct mm_struct* mm,
                              pgd_t* pgd,
                              unsigned long address) {
  return 0;
}
...
static inline p4d_t* p4d_alloc(struct mm_struct* mm,
                               pgd_t* pgd,
                               unsigned long address) {
...
static inline pud_t* pud_alloc(struct mm_struct* mm,
                               p4d_t* p4d,
                               unsigned long address) {
...
static inline pmd_t* pmd_alloc(struct mm_struct* mm,
                               pud_t* pud,
                               unsigned long address) {
...
pte_t* pte_offset_map_lock(struct mm_struct* mm,
                           pmd_t* pmd,
                           unsigned long addr,
                           spinlock_t** ptlp);

pte_t* pte_offset_map_ro_nolock(struct mm_struct* mm,
                                pmd_t* pmd,
                                unsigned long addr,
                                spinlock_t** ptlp);
pte_t* pte_offset_map_rw_nolock(struct mm_struct* mm,
                                pmd_t* pmd,
                                unsigned long addr,
                                pmd_t* pmdvalp,
                                spinlock_t** ptlp);
 ...
 */
extern unsigned long free_reserved_area(void* start,
                                        void* end,
                                        int poison,
                                        const char* s);
...
extern __printf(3, 4) void warn_alloc(gfp_t gfp_mask,
                                      const nodemask_t* nodemask,
                                      const char* fmt,
                                      ...);
...
void mapping_rmap_tree_remove(struct vm_area_struct* vma,
                              struct address_space* mapping);
struct vm_area_struct* mapping_rmap_tree_iter_first(
    struct address_space* mapping,
    pgoff_t pgoff_start,
    pgoff_t pgoff_last);
struct vm_area_struct* mapping_rmap_tree_iter_next(struct vm_area_struct* vma,
                                                   pgoff_t pgoff_start,
                                                   pgoff_t pgoff_last);

#define mapping_rmap_tree_foreach(vma, mapping, pgoff_start, pgoff_last)     \
  for (vma = mapping_rmap_tree_iter_first(mapping, pgoff_start, pgoff_last); \
       vma; vma = mapping_rmap_tree_iter_next(vma, pgoff_start, pgoff_last))

void anon_rmap_tree_insert(struct anon_vma_chain* avc,
                           struct anon_vma* anon_vma);
void anon_rmap_tree_remove(struct anon_vma_chain* avc,
                           struct anon_vma* anon_vma);
struct anon_vma_chain* anon_rmap_tree_iter_first(struct anon_vma* anon_vma,
                                                 pgoff_t pgoff_start,
                                                 pgoff_t pgoff_last);
struct anon_vma_chain* anon_rmap_tree_iter_next(struct anon_vma_chain* avc,
                                                pgoff_t pgoff_start,
                                                pgoff_t pgoff_last);
...
/* mmap.c */
extern int __vm_enough_memory(const struct mm_struct* mm,
                              long pages,
                              int cap_sys_admin);
...
unsigned long __get_unmapped_area(struct file* file,
                                  unsigned long addr,
                                  unsigned long len,
                                  unsigned long pgoff,
                                  unsigned long flags,
...
static inline unsigned long get_unmapped_area(struct file* file,
                                              ...
                                              unsigned long pgoff,
                                              unsigned long flags) {
  return __get_unmapped_area(file, addr, len, pgoff, flags, EMPTY_VMA_FLAGS);
}
...
unsigned long do_mmap(struct file* file,
                      ...
                      struct list_head* uf);
extern int do_vmi_munmap(struct vma_iterator* vmi,
                         struct mm_struct* mm,
                         unsigned long start,
                         size_t len,
                         struct list_head* uf,
...
extern int do_munmap(struct mm_struct*,
                     ...
                     size_t,
                     struct list_head* uf);
extern int do_madvise(struct mm_struct* mm,
                      unsigned long start,
                      size_t len_in,
                      int behavior);

#ifdef CONFIG_MMU
extern int __mm_populate(unsigned long addr,
                         unsigned long len,
                         int ignore_errors);
...
int __must_check vm_brk_flags(unsigned long addr,
                              unsigned long request,
                              bool is_exec);
int vm_munmap(unsigned long start, size_t len);
unsigned long __must_check vm_mmap(struct file* file,
                                   unsigned long addr,
                                   unsigned long len,
                                   unsigned long prot,
                                   unsigned long flag,
...
static inline unsigned long stack_guard_start_gap(
    const struct vm_area_struct* vma) {
...
/**
 * mmap_action_map_kernel_pages_full - helper for mmap_prepare hook to specify
 * that kernel pages contained in the @pages array should be mapped to userland
...
static inline void mmap_action_map_kernel_pages_full(struct vm_area_desc* desc,
                                                     struct page** pages) {
  mmap_action_map_kernel_pages(desc, desc->start, pages, vma_desc_pages(desc));
}
...
int vm_insert_pages(struct vm_area_struct* vma,
                    unsigned long addr,
                    struct page** pages,
                    unsigned long* num);
...
int map_kernel_pages_complete(struct vm_area_struct* vma,
                              struct mmap_action* action);
int vm_map_pages(struct vm_area_struct* vma,
                 struct page** pages,
                 unsigned long num);
int vm_map_pages_zero(struct vm_area_struct* vma,
                      struct page** pages,
                      unsigned long num);
vm_fault_t vmf_insert_page_mkwrite(struct vm_fault* vmf,
                                   struct page* page,
                                   bool write);
...
static inline bool set_page_guard(struct zone* zone,
                                  struct page* page,
                                  unsigned int order) {
  if (!debug_guardpage_enabled())
    return false;
...
void __clear_page_guard(struct zone* zone,
                        struct page* page,
                        unsigned int order);
static inline void clear_page_guard(struct zone* zone,
                                    struct page* page,
                                    unsigned int order) {
  if (!debug_guardpage_enabled())
    return;
...
struct vmem_altmap;
void* vmemmap_alloc_block_buf(unsigned long size,
                              int node,
                              struct vmem_altmap* altmap);
void vmemmap_verify(pte_t*, int, unsigned long, unsigned long);
void vmemmap_set_pmd(pmd_t* pmd,
                     void* p,
                     int node,
                     unsigned long addr,
                     unsigned long next);
int vmemmap_check_pmd(pmd_t* pmd,
                      int node,
                      unsigned long addr,
                      unsigned long next);
int vmemmap_populate_basepages(unsigned long start,
                               unsigned long end,
                               int node,
                               struct vmem_altmap* altmap);
int vmemmap_populate_hugepages(unsigned long start,
                               unsigned long end,
                               int node,
                               struct vmem_altmap* altmap);
int vmemmap_populate(unsigned long start,
                     unsigned long end,
                     int node,
                     struct vmem_altmap* altmap);
int vmemmap_populate_hvo(unsigned long start,
                         unsigned long end,
                         unsigned int order,
                         struct zone* zone,
                         unsigned long headsize);
void vmemmap_wrprotect_hvo(unsigned long start,
                           unsigned long end,
                           int node,
                           unsigned long headsize);
void vmemmap_populate_print_last(void);
#ifdef CONFIG_MEMORY_HOTPLUG
void vmemmap_free(unsigned long start,
                  unsigned long end,
                  struct vmem_altmap* altmap);
#endif

#ifdef CONFIG_SPARSEMEM_VMEMMAP
static inline unsigned long vmem_altmap_offset(
    const struct vmem_altmap* altmap) {
...
int mf_dax_kill_procs(struct address_space* mapping,
                      pgoff_t index,
                      unsigned long count,
                      int mf_flags);
...
int copy_user_large_folio(struct folio* dst,
                          struct folio* src,
...
int set_anon_vma_name(unsigned long addr,
                      unsigned long size,
                      const char __user* uname);
#else
static inline int set_anon_vma_name(unsigned long addr,
                                    unsigned long size,
                                    const char __user* uname) {
  return -EINVAL;
}
...
int reserve_mem_find_by_name(const char* name,
                             phys_addr_t* start,
                             phys_addr_t* size);
...
static inline bool user_alloc_needs_zeroing(void) {
  ...
  return cpu_dcache_is_aliasing() || cpu_icache_is_aliasing() ||
         !static_branch_maybe(CONFIG_INIT_ON_ALLOC_DEFAULT_ON, &init_on_alloc);
}
 ...
 */
#define PP_DMA_INDEX_BITS \
  MIN(32, __ffs(POISON_POINTER_DELTA) - PP_DMA_INDEX_SHIFT)
#else
/* Use the lowest bit of PAGE_OFFSET if there's at least 8 bits available; see
 * above */
...
#define PP_DMA_INDEX_MASK \
  GENMASK(PP_DMA_INDEX_BITS + PP_DMA_INDEX_SHIFT - 1, PP_DMA_INDEX_SHIFT)
...
void map_anon_folio_pte_nopf(struct folio* folio,
                             pte_t* pte,
                             struct vm_area_struct* vma,
                             unsigned long addr,
                             bool uffd_wp);

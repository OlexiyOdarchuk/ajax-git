...
struct buffer_head;
typedef int(get_block_t)(struct inode* inode,
                         sector_t iblock,
                         struct buffer_head* bh_result,
                         int create);
typedef int(dio_iodone_t)(struct kiocb* iocb,
                          loff_t offset,
                          ssize_t bytes,
                          void* private);
 ...
 */
#define FMODE_FSNOTIFY_MASK (FMODE_NONOTIFY | FMODE_NONOTIFY_PERM)
...
#define FMODE_FSNOTIFY_ACCESS_PERM(mode) ((mode & FMODE_FSNOTIFY_MASK) == 0)
#else
...
#define ATTR_DELEG \
  (1 << 18) /* Delegated attrs. Don't break write delegations */
...
#define TRACE_IOCB_STRINGS                                               \
  {IOCB_HIPRI, "HIPRI"}, {IOCB_DSYNC, "DSYNC"}, {IOCB_SYNC, "SYNC"},     \
      {IOCB_NOWAIT, "NOWAIT"}, {IOCB_APPEND, "APPEND"},                  \
...
struct address_space_operations {
  ...
  int (*write_begin)(const struct kiocb*,
                     struct address_space* mapping,
                     loff_t pos,
                     unsigned len,
                     struct folio** foliop,
                     void** fsdata);
  int (*write_end)(const struct kiocb*,
                   struct address_space* mapping,
                   loff_t pos,
                   unsigned len,
                   unsigned copied,
   ...
   */
  int (*migrate_folio)(struct address_space*,
                       struct folio* dst,
                       struct folio* src,
                       enum migrate_mode);
 ...
 */
static inline bool mapping_tagged(const struct address_space* mapping,
                                  xa_mark_t tag) {
...
static inline struct posix_acl* uncached_acl_sentinel(
 ...
 */
static inline enum inode_state_flags_enum inode_state_read_once(
    struct inode* inode) {
  return READ_ONCE(inode->i_state.__state);
}

static inline enum inode_state_flags_enum inode_state_read(
    struct inode* inode) {
...
static inline void inode_state_replace_raw(
    struct inode* inode,
...
static inline void inode_set_cached_link(struct inode* inode,
                                         char* link,
                                         int linklen) {
...
static inline void filemap_invalidate_lock_shared(
    struct address_space* mapping) {
...
extern struct fasync_struct* fasync_insert_entry(int,
                                                 struct file*,
                                                 struct fasync_struct**,
                                                 struct fasync_struct*);
...
extern void __f_setown(struct file* filp,
                       struct pid*,
                       enum pid_type,
                       int force);
...
static inline void i_uid_update(struct mnt_idmap* idmap,
  ...
  if (attr->ia_valid & ATTR_UID)
    inode->i_uid = from_vfsuid(idmap, i_user_ns(inode), attr->ia_vfsuid);
}
...
static inline void i_gid_update(struct mnt_idmap* idmap,
  ...
  if (attr->ia_valid & ATTR_GID)
    inode->i_gid = from_vfsgid(idmap, i_user_ns(inode), attr->ia_vfsgid);
}
...
struct timespec64 inode_set_ctime_to_ts(struct inode* inode,
                                        struct timespec64 ts);
 ...
 */
int vfs_create(struct mnt_idmap*,
               struct dentry*,
               umode_t,
               struct delegated_inode*);
struct dentry* vfs_mkdir(struct mnt_idmap*,
                         struct inode*,
                         struct dentry*,
                         umode_t,
                         struct delegated_inode*);
int vfs_mknod(struct mnt_idmap*,
              struct inode*,
              struct dentry*,
              umode_t,
              dev_t,
              struct delegated_inode*);
int vfs_symlink(struct mnt_idmap*,
                struct inode*,
                struct dentry*,
                const char*,
                struct delegated_inode*);
int vfs_link(struct dentry*,
             ...
             struct dentry*,
             struct delegated_inode*);
int vfs_rmdir(struct mnt_idmap*,
              struct inode*,
              struct dentry*,
              struct delegated_inode*);
int vfs_unlink(struct mnt_idmap*,
               struct inode*,
               struct dentry*,
               struct delegated_inode*);
...
static inline int vfs_whiteout(struct mnt_idmap* idmap,
  ...
  return vfs_mknod(idmap, dir, dentry, S_IFCHR | WHITEOUT_MODE, WHITEOUT_DEV,
                   NULL);
}
...
struct file* kernel_file_open(const struct path* path,
                              int flags,
                              const struct cred* cred);

int vfs_mkobj(struct dentry*,
              umode_t,
...
#ifdef CONFIG_COMPAT
extern long compat_ptr_ioctl(struct file* file,
                             unsigned int cmd,
                             unsigned long arg);
#else
...
struct dir_context;
typedef bool (
    *filldir_t)(struct dir_context*, const char*, int, loff_t, u64, unsigned);
...
#endif
  ssize_t (*copy_file_range)(struct file*,
                             loff_t,
                             struct file*,
                             loff_t,
                             size_t,
                             unsigned int);
  loff_t (*remap_file_range)(struct file* file_in,
                             loff_t pos_in,
...
int wrap_directory_iterator(struct file*,
                            struct dir_context*,
...
struct inode_operations {
  ...
  struct dentry* (*mkdir)(struct mnt_idmap*,
                          struct inode*,
                          struct dentry*,
                          umode_t);
  int (*rmdir)(struct inode*, struct dentry*);
  int (
      *mknod)(struct mnt_idmap*, struct inode*, struct dentry*, umode_t, dev_t);
  ...
  int (*update_time)(struct inode* inode,
                     enum fs_update_time type,
                     unsigned int flags);
  void (*sync_lazytime)(struct inode* inode);
  int (*atomic_open)(struct inode*,
                     struct dentry*,
                     struct file*,
                     unsigned open_flag,
                     umode_t create_mode);
...
static inline int vfs_mmap_prepare(struct file* file,
...
extern ssize_t vfs_copy_file_range(struct file*,
                                   loff_t,
                                   struct file*,
                                   loff_t,
                                   size_t,
...
int __generic_remap_file_range_prep(struct file* file_in,
                                    loff_t pos_in,
                                    struct file* file_out,
                                    loff_t pos_out,
                                    loff_t* len,
...
int generic_remap_file_range_prep(struct file* file_in,
                                  loff_t pos_in,
                                  struct file* file_out,
                                  loff_t pos_out,
                                  loff_t* count,
                                  unsigned int remap_flags);
extern loff_t vfs_clone_file_range(struct file* file_in,
                                   loff_t pos_in,
                                   struct file* file_out,
                                   loff_t pos_out,
                                   loff_t len,
...
extern loff_t vfs_dedupe_file_range_one(struct file* src_file,
                                        loff_t src_pos,
...
/*
 * returns the refcount on the inode. The lock guarantees no 0->1 or 1->0
 * transitions of the count are going to take place, otherwise it changes
 * arbitrarily.
 */
...
#define FS_USERNS_MOUNT_RESTRICTED \
  512 /* Restrict mount in userns if not already visible */
#define FS_USERNS_DELEGATABLE \
  1024 /* Can be mounted inside userns from outside */
...
__printf(2, 3) static inline void super_set_sysfs_name_generic(
...
int inode_update_time(struct inode* inode,
                      enum fs_update_time type,
                      unsigned int flags);
int generic_update_time(struct inode* inode,
                        enum fs_update_time type,
                        unsigned int flags);
...
int do_truncate(struct mnt_idmap*,
                struct dentry*,
                loff_t start,
                unsigned int time_attrs,
                struct file* filp);
extern int vfs_fallocate(struct file* file,
                         int mode,
                         loff_t offset,
                         loff_t len);
...
struct file* dentry_open_nonotify(const struct path* path,
                                  int flags,
                                  const struct cred* cred);
struct file* dentry_create(struct path* path,
                           int flags,
                           umode_t mode,
                           const struct cred* cred);
...
static inline struct filename* getname_maybe_null(const char __user* name,
                                                  int flags) {
...
EXTEND_CLASS(filename,
             _maybe_null,
             getname_maybe_null(p, f),
             const char __user* p,
             unsigned int f)
EXTEND_CLASS(filename,
             _complete_delayed,
             complete_getname(p),
             struct delayed_filename* p)

extern int finish_open(struct file* file,
                       struct dentry* dentry,
                       int (*open)(struct inode*, struct file*));
...
extern int __must_check file_fdatawait_range(struct file* file,
                                             loff_t lstart,
                                             loff_t lend);
...
extern int __must_check file_write_and_wait_range(struct file* file,
                                                  loff_t start,
                                                  loff_t end);
int filemap_flush_range(struct address_space* mapping,
                        loff_t start,
                        loff_t end);
...
extern int vfs_fsync_range(struct file* file,
                           loff_t start,
                           loff_t end,
                           int datasync);
extern int vfs_fsync(struct file* file, int datasync);

extern int sync_file_range(struct file* file,
                           loff_t offset,
                           loff_t nbytes,
                           unsigned int flags);
...
static inline ssize_t generic_write_sync(struct kiocb* iocb, ssize_t count) {
  if (iocb_is_dsync(iocb)) {
    int ret =
        vfs_fsync_range(iocb->ki_filp, iocb->ki_pos - count, iocb->ki_pos - 1,
...
int notify_change(struct mnt_idmap*,
                  struct dentry*,
                  struct iattr*,
                  struct delegated_inode*);
...
static inline int path_permission(const struct path* path, int mask) {
  return inode_permission(mnt_idmap(path->mnt), d_inode(path->dentry), mask);
}
int __check_sticky(struct mnt_idmap* idmap,
                   struct inode* dir,
                   struct inode* inode);

int may_delete_dentry(struct mnt_idmap* idmap,
                      struct inode* dir,
...
int inode_init_always_gfp(struct super_block* sb,
                          struct inode* inode,
                          gfp_t gfp);
static inline int inode_init_always(struct super_block* sb,
                                    struct inode* inode) {
...
struct inode* iget5_locked(struct super_block* sb,
                           ...
                           int (*set)(struct inode*, void*),
                           void* data);
struct inode* iget5_locked_rcu(struct super_block* sb,
                               u64 hashval,
                               int (*test)(struct inode*, void*),
                               int (*set)(struct inode*, void*),
                               void* data);
...
struct inode* find_inode_nowait(struct super_block* sb,
                                ...
                                void* data);
struct inode* find_inode_rcu(struct super_block* sb,
                             u64 hashval,
                             int (*test)(struct inode*, void*),
                             void* data);
...
extern int generic_write_check_limits(struct file* file,
                                      loff_t pos,
                                      loff_t* count);
extern int generic_file_rw_checks(struct file* file_in, struct file* file_out);
ssize_t filemap_read(struct kiocb* iocb,
                     struct iov_iter* to,
...
ssize_t direct_write_fallback(struct kiocb* iocb,
                              struct iov_iter* iter,
                              ssize_t direct_written,
                              ssize_t buffered_written);

ssize_t vfs_iter_read(struct file* file,
                      struct iov_iter* iter,
                      loff_t* ppos,
                      rwf_t flags);
ssize_t vfs_iter_write(struct file* file,
                       struct iov_iter* iter,
                       loff_t* ppos,
                       rwf_t flags);
ssize_t vfs_iocb_iter_read(struct file* file,
                           struct kiocb* iocb,
                           struct iov_iter* iter);
ssize_t vfs_iocb_iter_write(struct file* file,
                            struct kiocb* iocb,
                            struct iov_iter* iter);

/* fs/splice.c */
ssize_t filemap_splice_read(struct file* in,
                            loff_t* ppos,
                            struct pipe_inode_info* pipe,
                            size_t len,
                            unsigned int flags);
ssize_t copy_splice_read(struct file* in,
                         loff_t* ppos,
                         struct pipe_inode_info* pipe,
                         size_t len,
                         unsigned int flags);
...
extern loff_t generic_file_llseek_size(struct file* file,
                                       loff_t offset,
                                       int whence,
                                       loff_t maxsize,
                                       loff_t eof);
loff_t generic_llseek_cookie(struct file* file,
                             loff_t offset,
                             int whence,
                             u64* cookie);
extern loff_t fixed_size_llseek(struct file* file,
                                loff_t offset,
                                int whence,
                                loff_t size);
...
#ifdef CONFIG_BLOCK
typedef void(dio_submit_t)(struct bio* bio,
                           struct inode* inode,
                           loff_t file_offset);
...
ssize_t __blockdev_direct_IO(struct kiocb* iocb,
                             struct inode* inode,
...
extern void inode_set_flags(struct inode* inode,
                            unsigned int flags,
                            unsigned int mask);
...
extern const char* page_get_link_raw(struct dentry*,
                                     struct inode*,
                                     struct delayed_call*);
extern const char* page_get_link(struct dentry*,
                                 struct inode*,
                                 struct delayed_call*);
...
extern int vfs_getattr_nosec(const struct path*,
                             struct kstat*,
                             u32,
                             unsigned int);
...
const char* simple_get_link(struct dentry*,
                            struct inode*,
                            struct delayed_call*);
...
extern int simple_getattr(struct mnt_idmap*,
                          const struct path*,
                          struct kstat*,
                          u32,
                          unsigned int);
...
void simple_rename_timestamp(struct inode* old_dir,
                             struct dentry* old_dentry,
...
extern int simple_rename_exchange(struct inode* old_dir,
                                  ...
                                  struct inode* new_dir,
                                  struct dentry* new_dentry);
extern int simple_rename(struct mnt_idmap*,
                         struct inode*,
                         struct dentry*,
                         struct inode*,
                         struct dentry*,
...
extern void simple_remove_by_name(struct dentry*,
                                  const char*,
                                  void (*callback)(struct dentry*));
...
extern struct dentry* simple_lookup(struct inode*,
                                    struct dentry*,
                                    unsigned int flags);
...
extern int simple_fill_super(struct super_block*,
                             unsigned long,
                             const struct tree_descr*);
extern int simple_pin_fs(struct file_system_type*,
...
extern ssize_t simple_read_from_buffer(void __user* to,
                                       ...
                                       const void* from,
                                       size_t available);
extern ssize_t simple_write_to_buffer(void* to,
                                      size_t available,
                                      loff_t* ppos,
...
void simple_offset_rename(struct inode* old_dir,
                          struct dentry* old_dentry,
                          struct inode* new_dir,
                          struct dentry* new_dentry);
...
int may_setattr(struct mnt_idmap* idmap,
                struct inode* inode,
                unsigned int ia_valid);
int setattr_prepare(struct mnt_idmap*, struct dentry*, struct iattr*);
extern int inode_newsize_ok(const struct inode*, loff_t offset);
void setattr_copy(struct mnt_idmap*,
                  struct inode* inode,
                  const struct iattr* attr);
...
static inline int kiocb_set_rw_flags(struct kiocb* ki,
                                     rwf_t flags,
                                     int rw_type) {
  int kiocb_flags = 0;
...
ssize_t simple_attr_write(struct file* file,
                          ...
                          size_t len,
                          loff_t* ppos);
ssize_t simple_attr_write_signed(struct file* file,
                                 const char __user* buf,
                                 size_t len,
                                 loff_t* ppos);
...
static inline bool dir_emit_dot(struct file* file, struct dir_context* ctx) {
  return ctx->actor(ctx, ".", 1, ctx->pos, file->f_path.dentry->d_inode->i_ino,
                    DT_DIR);
}
static inline bool dir_emit_dotdot(struct file* file, struct dir_context* ctx) {
  return ctx->actor(ctx, "..", 2, ctx->pos, d_parent_ino(file->f_path.dentry),
                    DT_DIR);
}
...
/* mm/fadvise.c */
extern int vfs_fadvise(struct file* file,
                       loff_t offset,
                       loff_t len,
                       int advice);
extern int generic_fadvise(struct file* file,
                           loff_t offset,
                           loff_t len,
                           int advice);

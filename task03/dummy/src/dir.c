...
int __ext4_check_dir_entry(const char* function,
  ...
  const int rlen = ext4_rec_len_from_disk(de->rec_len, dir->i_sb->s_blocksize);
  ...
  else if (unlikely(rlen < ext4_dir_rec_len(de->name_len, fake ? NULL : dir)))
    error_msg = "rec_len is too small for name_len";
  else if (unlikely(next_offset > size))
    error_msg = "directory entry overrun";
  else if (unlikely(next_offset >
                        size - ext4_dir_rec_len(1, has_csum ? NULL : dir) &&
                    next_offset != size))
  ...
  if (filp)
    ext4_error_file(filp, function, line, bh->b_blocknr,
                    ...
                    error_msg, offset, le32_to_cpu(de->inode), rlen, size,
                    fake);
  else
...
static int ext4_readdir(struct file* file, struct dir_context* ctx) {
  ...
  while (ctx->pos < inode->i_size) {
    ...
    if (err > 0) {
      pgoff_t index = map.m_pblk << inode->i_blkbits >> PAGE_SHIFT;
      if (!ra_has_index(&file->f_ra, index))
        page_cache_sync_readahead(sb->s_bdev->bd_mapping, &file->f_ra, file,
                                  index, 1 << EXT4_SB(sb)->s_min_folio_order);
    ...
    /* Check the checksum */
    if (!buffer_verified(bh) && !ext4_dirblock_csum_verify(inode, bh)) {
    ...
    if (!inode_eq_iversion(inode, info->cookie)) {
      for (i = 0; i <= sb->s_blocksize -
         ...
         * dirent test below. */
        if (ext4_rec_len_from_disk(de->rec_len, sb->s_blocksize) <
            ext4_dir_rec_len(1, inode))
          break;
    ...
    if (unlikely(offset < sb->s_blocksize &&
      ...
      EXT4_ERROR_FILE(file, bh->b_blocknr,
                      ...
                      "directory entry too close to block end", offset,
                      sb->s_blocksize);
    ...
    while (ctx->pos < inode->i_size && offset < sb->s_blocksize) {
      ...
      if (ext4_check_dir_entry(inode, file, de, bh, bh->b_data, bh->b_size,
                               offset)) {
        /*
      ...
      if (le32_to_cpu(de->inode)) {
        if (!IS_ENCRYPTED(inode)) {
          ...
          err = fscrypt_fname_disk_to_usr(inode, hash, minor_hash, &de_name,
                                          &fstr);
          de_name = fstr;
          fstr.len = save_len;
          if (err)
            goto errout;
          if (!dir_emit(ctx, de_name.name, de_name.len, le32_to_cpu(de->inode),
                        get_dtype(sb, de->file_type)))
            goto done;
        }
      }
      ctx->pos += ext4_rec_len_from_disk(de->rec_len, sb->s_blocksize);
    }
  next_block:
...
static loff_t ext4_dir_llseek(struct file* file, loff_t offset, int whence) {
  ...
  if (likely(dx_dir))
    ret = generic_file_llseek_size(file, offset, whence, htree_max, htree_max);
  else
...
static void free_rb_tree_fname(struct rb_root* root) {
  ...
  rbtree_postorder_for_each_entry_safe(fname, next, root,
                                       rb_hash) while (fname) {
 ...
 */
static int call_filldir(struct file* file,
...
static int ext4_dx_readdir(struct file* file, struct dir_context* ctx) {
  ...
  while (1) {
     ...
     * cached entries.
     */
    if ((!info->curr_node) || !inode_eq_iversion(inode, info->cookie)) {
      info->curr_node = NULL;
      free_rb_tree_fname(&info->root);
      info->cookie = inode_query_iversion(inode);
      ret = ext4_htree_fill_tree(file, info->curr_hash, info->curr_minor_hash,
                                 &info->next_hash);
      if (ret < 0)
        goto finished;
      if (ret == 0) {
...
int ext4_check_all_de(struct inode* dir,
                      struct buffer_head* bh,
                      void* buf,
                      int buf_size) {
  struct ext4_dir_entry_2* de;
  ...
  top = buf + buf_size;
  while ((char*)de < top) {
    if (ext4_check_dir_entry(dir, NULL, de, bh, buf, buf_size, offset))
      return -EFSCORRUPTED;

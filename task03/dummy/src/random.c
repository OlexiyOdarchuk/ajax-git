...
/*
 * Copyright (C) 2017-2024 Jason A. Donenfeld <Jason@zx2c4.com>. All Rights
 * Reserved. Copyright Matt Mackall <mpm@selenic.com>, 2003, 2004, 2005
 * Copyright Theodore Ts'o, 1994, 1995, 1996, 1997, 1998, 1999. All rights
 * reserved.
 *
 * This driver produces cryptographically secure pseudorandom data. It is
 * divided into roughly six sections, each with a section header:
 *
...
static struct ratelimit_state urandom_warning =
    RATELIMIT_STATE_INIT_FLAGS("urandom_warning",
                               HZ,
                               3,
...
int __cold execute_with_initialized_rng(struct notifier_block* nb) {
  ...
  if (crng_ready())
    ...
    ret = raw_notifier_chain_register(
        (struct raw_notifier_head*)&random_ready_notifier.head, nb);
...
static void crng_reseed(struct work_struct* work) {
  ...
  /* Immediately schedule the next reseeding, so that it fires sooner rather
   * than later. */
...
static void crng_make_state(struct chacha_state* chacha_state,
  ...
  if (!crng_ready()) {
    ...
    if (!ready) {
      ...
      crng_fast_key_erasure(base_crng.key, chacha_state, random_data,
                            random_data_len);
    }
    spin_unlock_irqrestore(&base_crng.lock, flags);
    if (!ready)
...
static struct {
  ...
  unsigned int init_bits;
    ...
    .hash.h = {BLAKE2S_IV0 ^ (0x01010000 | BLAKE2S_HASH_SIZE), BLAKE2S_IV1,
               BLAKE2S_IV2, BLAKE2S_IV3, BLAKE2S_IV4, BLAKE2S_IV5, BLAKE2S_IV6,
               BLAKE2S_IV7},
...
static void extract_entropy(void* buf, size_t len) {
  ...
  for (i = 0; i < ARRAY_SIZE(block.rdseed);) {
    ...
    longs =
        arch_get_random_longs(&block.rdseed[i], ARRAY_SIZE(block.rdseed) - i);
    if (longs) {
      i += longs;
      continue;
    }
  ...
  block.counter = 0;
  blake2s(seed, sizeof(seed), (const u8*)&block, sizeof(block), next_key,
          sizeof(next_key));
 ...
 *entropy, bool sleep_after); void add_bootloader_randomness(const void *buf,
 *size_t len); void add_vmfork_randomness(const void *unique_vm_id, size_t len);
 *	void add_interrupt_randomness(int irq);
 *	void add_input_randomness(unsigned int type, unsigned int code, unsigned
 *int value); void add_disk_randomness(struct gendisk *disk);
 *
...
void __init random_init(void) {
  ...
  WARN(!entropy,
       "Missing cycle counter and fallback timer; RNG "
 ...
 */
void add_hwgenerator_randomness(const void* buf,
                                size_t len,
                                size_t entropy,
                                bool sleep_after) {
...
static void __cold try_to_generate_entropy(void) {
  ...
  struct entropy_timer_state* stack =
      PTR_ALIGN((void*)stack_bytes, SMP_CACHE_BYTES);
  ...
  while (!crng_ready() && !signal_pending(current)) {
    /*
     * Check !timer_pending() and then ensure that any previous callback has
     * finished executing by checking timer_delete_sync_try(), before queueing
     * the next one.
     */
    if (!timer_pending(&stack->timer) &&
        timer_delete_sync_try(&stack->timer) >= 0) {
 ...
 */
static int proc_do_uuid(const struct ctl_table* table,
                        int write,
                        void* buf,
                        size_t* lenp,
                        loff_t* ppos) {
...
static int proc_do_rointvec(const struct ctl_table* table,
                            int write,
                            void* buf,
                            size_t* lenp,
                            loff_t* ppos) {

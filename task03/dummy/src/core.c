...
static int __fwnode_link_add(struct fwnode_handle* con,
  ...
  list_for_each_entry(link, &sup->consumers,
                      s_hook) if (link->consumer == con) {
    link->flags |= flags;
    return 0;
  }
 ...
 */
static void __fw_devlink_pickup_dangling_consumers(
...
#define DL_MARKER_FLAGS (DL_FLAG_INFERRED | DL_FLAG_CYCLE | DL_FLAG_MANAGED)
...
static int devlink_add_symlinks(struct device* dev) {
  ...
  buf_con =
      kasprintf(GFP_KERNEL, "consumer:%s:%s", dev_bus_name(con), dev_name(con));
  if (!buf_con) {
    ret = -ENOMEM;
    goto err_con_dev;
  }
...
static void devlink_remove_symlinks(struct device* dev) {
  ...
  if (device_is_registered(con)) {
    buf_sup = kasprintf(GFP_KERNEL, "supplier:%s:%s", dev_bus_name(sup),
                        dev_name(sup));
    if (!buf_sup)
      goto out;
...
#define DL_MANAGED_LINK_FLAGS                                                \
  (DL_FLAG_AUTOREMOVE_CONSUMER | DL_FLAG_AUTOREMOVE_SUPPLIER |               \
   DL_FLAG_AUTOPROBE_CONSUMER | DL_FLAG_SYNC_STATE_ONLY | DL_FLAG_INFERRED | \
   DL_FLAG_CYCLE)
...
struct device_link* device_link_add(struct device* consumer,
  ...
  list_for_each_entry(link, &supplier->links.consumers, s_node) {
    ...
    if (device_link_test(link, DL_FLAG_INFERRED) && !(flags & DL_FLAG_INFERRED))
  ...
  dev_set_name(&link->link_dev, "%s:%s--%s:%s", dev_bus_name(supplier),
               dev_name(supplier), dev_bus_name(consumer), dev_name(consumer));
   ...
   */
  if (link->status == DL_STATE_CONSUMER_PROBE && flags & DL_FLAG_PM_RUNTIME)
...
static bool dev_is_best_effort(struct device* dev) {
  return (fw_devlink_best_effort && dev_can_match(dev)) ||
         (dev->fwnode &&
          fwnode_test_flag(dev->fwnode, FWNODE_FLAG_BEST_EFFORT));
}
...
static struct fwnode_handle* fwnode_links_check_suppliers(
  ...
  list_for_each_entry(
      link, &fwnode->suppliers,
      c_hook) if (!(link->flags & (FWLINK_FLAG_CYCLE |
                                   FWLINK_FLAG_IGNORE))) return link->supplier;
...
int device_links_check_suppliers(struct device* dev) {
  ...
  scoped_guard(mutex, &fwnode_link_lock) {
    ...
    if (sup_fw) {
      if (dev_is_best_effort(dev))
        ...
        return dev_err_probe(dev, -EPROBE_DEFER, "wait for supplier %pfwf\n",
                             sup_fw);
    }
  }
  ...
  list_for_each_entry(link, &dev->links.suppliers, c_node) {
    ...
    if (link->status != DL_STATE_AVAILABLE &&
        !device_link_test(link, DL_FLAG_SYNC_STATE_ONLY)) {
      if (dev_is_best_effort(dev) && device_link_test(link, DL_FLAG_INFERRED) &&
...
static ssize_t waiting_for_supplier_show(struct device* dev,
  ...
  device_lock(dev);
  scoped_guard(mutex, &fwnode_link_lock) val =
      !!fwnode_links_check_suppliers(dev->fwnode);
  device_unlock(dev);
...
start:
  ...
  list_for_each_entry(link, &dev->links.consumers, s_node) {
    ...
    if (status == DL_STATE_ACTIVE) {
      ...
      device_release_driver_internal(consumer, NULL, consumer->parent);
      put_device(consumer);
      goto start;
    }
  }
...
static void device_links_purge(struct device* dev) {
  ...
  list_for_each_entry_safe_reverse(link, ln, &dev->links.consumers, s_node) {
    WARN_ON(link->status != DL_STATE_DORMANT && link->status != DL_STATE_NONE);
    __device_link_del(&link->kref);
  }
...
void fw_devlink_drivers_done(void) {
  ...
  device_links_write_lock();
  class_for_each_device(&devlink_class, NULL, NULL, fw_devlink_no_driver);
  device_links_write_unlock();
}
...
static bool __fw_devlink_relax_cycles(struct fwnode_handle* con_handle,
   ...
   * further.
   */
  if (sup_dev && sup_dev->links.status == DL_DEV_DRIVER_BOUND && con_dev &&
  ...
  list_for_each_entry(dev_link, &sup_dev->links.suppliers, c_node) {
    ...
    if (__fw_devlink_relax_cycles(con_handle, dev_link->supplier->fwnode)) {
      pr_debug("%pfwf: cycle: depends on %pfwf\n", sup_handle,
...
static void __fw_devlink_link_to_consumers(struct device* dev) {
  ...
  list_for_each_entry_safe(link, tmp, &fwnode->consumers, s_hook) {
    ...
    if (!con_dev) {
       ...
       */
      if (con_dev && fwnode_is_ancestor_of(con_dev->fwnode, fwnode)) {
        put_device(con_dev);
        con_dev = NULL;
      } else {
        own_link = false;
...
ssize_t device_store_bool(struct device* dev,
                          struct device_attribute* attr,
                          const char* buf,
                          size_t size) {
  struct dev_ext_attribute* ea = to_ext_attr(attr);
...
ssize_t device_show_bool(struct device* dev,
                         struct device_attribute* attr,
                         char* buf) {
  struct dev_ext_attribute* ea = to_ext_attr(attr);
...
static void device_release(struct kobject* kobj) {
  ...
  if (dev->release)
    ...
    WARN(1,
         KERN_ERR
         "Device '%s' does not have a release() function, it is broken and "
...
static void device_get_ownership(const struct kobject* kobj,
                                 kuid_t* uid,
                                 kgid_t* gid) {
  const struct device* dev = kobj_to_dev(kobj);
 ...
 */
static void dev_driver_uevent(const struct device* dev,
...
static ssize_t uevent_show(struct device* dev,
                           const struct device_attribute* attr,
                           char* buf) {
...
static ssize_t uevent_store(struct device* dev,
                            const struct device_attribute* attr,
                            const char* buf,
                            size_t count) {
  int rc;
...
static ssize_t removable_show(struct device* dev,
                              const struct device_attribute* attr,
                              char* buf) {
  const char* loc;
...
int devm_device_add_group(struct device* dev,
  ...
  devres = devres_alloc(devm_attr_group_remove, sizeof(*devres), GFP_KERNEL);
  if (!devres)
    return -ENOMEM;
...
static int device_add_attrs(struct device* dev) {
  ...
  if (dev_add_physical_location(dev)) {
    error = device_add_group(dev, &dev_attr_physical_location_group);
    if (error)
...
static const struct kobj_ns_type_operations* class_dir_child_ns_type(
...
static struct kobject* get_device_parent(struct device* dev,
  ...
  if (sp) {
    ...
    list_for_each_entry(k, &sp->glue_dirs.list,
                        entry) if (k->parent == parent_kobj) {
      kobj = kobject_get(k);
      break;
    }
...
static void cleanup_glue_dir(struct device* dev, struct kobject* glue_dir) {
   ...
   *                                                 kobject_add_internal()
   *                                                   create_dir() // in
   * glue_dir sysfs_create_dir_ns() kernfs_create_dir_ns(sd)
   *
...
int device_add(struct device* dev) {
  ...
  if (sp) {
    ...
    list_for_each_entry(class_intf, &sp->interfaces,
                        node) if (class_intf->add_dev) class_intf->add_dev(dev);
 ...
 * other than 0, we break out and return that value.
 */
int device_for_each_child_reverse(struct device* parent,
                                  void* data,
                                  device_iter_t fn) {
  struct klist_iter i;
  struct device* child;
...
struct device* device_create(const struct class* class,
  ...
  va_start(vargs, fmt);
  dev = device_create_groups_vargs(class, parent, devt, drvdata, NULL, fmt,
                                   vargs);
  va_end(vargs);
  return dev;
}
...
struct device* device_create_with_groups(const struct class* class,
  ...
  va_start(vargs, fmt);
  dev = device_create_groups_vargs(class, parent, devt, drvdata, groups, fmt,
                                   vargs);
  va_end(vargs);
  return dev;
}
...
out:
  if (error && is_link_renamed)
    sysfs_rename_link_ns(&sp->subsys.kobj, kobj, new_name, old_device_name,
...
int device_move(struct device* dev,
  ...
  pr_debug("device: '%s': %s: moving to '%s'\n", dev_name(dev), __func__,
           new_parent ? dev_name(new_parent) : "<NULL>");
...
static int device_attrs_change_owner(struct device* dev,
  ...
  if (class) {
     ...
     * @kuid/@kgid.
     */
    error = sysfs_groups_change_owner(kobj, class->dev_groups, kuid, kgid);
    if (error)
      return error;
  }
  ...
  if (type) {
     ...
     * @kuid/@kgid.
     */
    error = sysfs_groups_change_owner(kobj, type->groups, kuid, kgid);
    if (error)
      return error;
  }
  ...
  if (device_supports_offline(dev) && !dev_offline_disabled(dev)) {
    ...
    error =
        sysfs_file_change_owner(kobj, dev_attr_online.attr.name, kuid, kgid);
    if (error)
      return error;
  }
...
int device_change_owner(struct device* dev, kuid_t kuid, kgid_t kgid) {
   ...
   * that step here.
   */
  error = sysfs_file_change_owner(kobj, dev_attr_uevent.attr.name, kuid, kgid);
  if (error)
    goto out;
  ...
  error = sysfs_link_change_owner(&sp->subsys.kobj, &dev->kobj, dev_name(dev),
                                  kuid, kgid);
  subsys_put(sp);
...
static void set_dev_info(const struct device* dev,
  ...
  if (MAJOR(dev->devt)) {
    ...
    snprintf(dev_info->device, sizeof(dev_info->device), "%c%u:%u", c,
             MAJOR(dev->devt), MINOR(dev->devt));
  } else if (strcmp(subsys, "net") == 0) {
    struct net_device* net = to_net_dev(dev);

    snprintf(dev_info->device, sizeof(dev_info->device), "n%u", net->ifindex);
  } else {
    snprintf(dev_info->device, sizeof(dev_info->device), "+%s:%s", subsys,
             dev_name(dev));
  }
}

int dev_vprintk_emit(int level,
...
static void __dev_printk(const char* level,
                         const struct device* dev,
                         struct va_format* vaf) {
  if (dev)
    dev_printk_emit(level[1] - '0', dev, "%s %s: %pV", dev_driver_string(dev),
                    dev_name(dev), vaf);
  else
...
static void __dev_probe_failed(const struct device* dev,
                               int err,
                               bool fatal,
                               const char* fmt,
                               va_list vargsp) {

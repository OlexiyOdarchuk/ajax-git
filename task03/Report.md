...
1. Зміна стилю форматування коштує на два порядки дорожче за зміну версії
   форматера: 17800 рядків проти 34. Наслідок видно по `git blame`:

   ```console
   $ git blame task03/dummy/src/dir.c | awk '{print $1}' | sort | uniq -c | sort -rn
       519 10d698d8    TASK 03 : switch dummy to chromium config
       149 3b4d2976    TASK 03 : add dummy project
   ```

   78% рядків файлу приписані коміту форматування. Вціліли переважно коментарі
   та порожні рядки, яких форматер не торкнувся. Коміти зміни версії форматера
   в розподілі взагалі не з'явились - їхні 34 рядки розчинились у пізнішому
   переформатуванні. Тобто історія авторства коду після такого переходу
   фактично втрачена, і такий перехід має йти окремим комітом, нічим не
   розбавленим.
